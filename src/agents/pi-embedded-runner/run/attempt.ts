...
export async function runEmbeddedAttempt(
  ...
  try {
    ...
    try {
      ...
      // Safety net: repair tool_use/tool_result pairing after all context extensions
      // but before the API call. Catches corruption that accumulates during
      // long-running conversations (interrupted tool calls, extension edge cases).
      if (transcriptPolicy.repairToolUseResultPairing) {
        const mutableAgent = activeSession.agent as unknown as {
          transformContext?: (
            m: AgentMessage[],
            s: AbortSignal,
          ) => AgentMessage[] | Promise<AgentMessage[]>;
        };
        const origTransform = mutableAgent.transformContext;
        mutableAgent.transformContext = async (messages: AgentMessage[], signal: AbortSignal) => {
          const transformed = origTransform
            ? await origTransform.call(mutableAgent, messages, signal)
            : messages;
          return sanitizeToolUseResultPairing(transformed);
        };
      }
